...
local function ReadMusicKit(name)
	...
	if json.MainMenuMusic and not isstring(json.MainMenuMusic) then
		return "MainMenuMusic must be a string."
	end
	...
	if json.MainMenuMusic then
		comp.MainMenuMusic = json.MainMenuMusic
	end
...
local function GetMusicKits()
	...
	for v,k in pairs(kits) do
		...
		if err then
			...
			impulse.MenuMessage.Add("musickit-error", "Music Kit Error", "Error when loading '"..k.."' music kit.\nError: "..err, Color(255, 0, 0), "https://forums.landis-community.com/index.php?threads/music-kit-creation-guide.808/", "Read the music kit guide")
			continue
		end
	end
end
...
function PLUGIN:GetMainMenuMusic()
	if impulse.GetSetting("music_mainmenu") == "Custom" then
		local kitName = impulse.GetSetting("music_kit")

		if kitName and CUSTOM_MUSICKITS[kitName] and CUSTOM_MUSICKITS[kitName].MainMenuMusic then
			return CUSTOM_MUSICKITS[kitName].MainMenuMusic
		else
			impulse.MenuMessage.Add("musickit-error", "Music Kit Error", "You have selected custom main menu music in your settings, but your music kit doesn't have custom main menu music.")
		end
	end 
end
